...
exports.registerVendor = async (req, res) => {
  const { company_name, contact_person, phone, gst_details, category } = req.body;
...
exports.getVendors = async (req, res) => {
  try {
    const vendors = await db.query('SELECT v.*, u.email FROM vendors v JOIN users u ON v.user_id = u.id');
...
exports.getVendorById = async (req, res) => {
  try {
    const vendor = await db.query('SELECT v.*, u.email FROM vendors v JOIN users u ON v.user_id = u.id WHERE v.id = $1', [req.params.id]);
