...
exports.getRFQById = async (req, res) => {
  try {
    const rfq = await db.query('SELECT * FROM rfqs WHERE id = $1', [req.params.id]);
    if (rfq.rows.length === 0) return res.status(404).json({ message: 'RFQ not found' });
